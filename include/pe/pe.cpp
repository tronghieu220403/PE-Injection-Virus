...
DWORD GetImageBase32(PVOID data)
{
    ...
    else
    {
        PIMAGE_NT_HEADERS32 p_image_nt_headers_32;
        p_image_nt_headers_32 = (PIMAGE_NT_HEADERS32)((PUCHAR)data + p_image_dos_header->e_lfanew);
        return p_image_nt_headers_32->OptionalHeader.ImageBase;
    }
}
...
void GetFunctionAddresses(const PDATA data)
{
    ...
    for(unsigned int i = 0; i < p_image_export_directory->NumberOfNames; i++)
    {
        ...
        if (hash == 0x786a6cd)
        {
            data->iat->fnWriteConsoleA = (pWriteConsoleA)((PUCHAR)kernel32_base + function_table[ordinal[i]]);
        }

        // Hash of GetStdHandle
        if (hash == 0x34dcbbd3)
        {
            data->iat->fnGetStdHandle = (pGetStdHandle)((PUCHAR)kernel32_base + function_table[ordinal[i]]);
        }
